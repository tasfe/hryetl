 ...
 * @since 8-apr-2003
 */
public class TableInputVarchar extends BaseStep implements StepInterface {
	private static Class<?> PKG = TableInputVarcharMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!

	private TableInputVarcharMeta meta;
	private TableInputVarcharData data;

	public TableInputVarchar(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
			Trans trans) {
												...
												// input stream
			{
				...
				if (nextRow == null) { // Nothing more to get!
					...
					if (data.thisrow != null) {
						putRow(data.rowMeta, data.thisrow); // fill the
															// rowset(s). (wait
															// for empty)
	...
	private boolean doQuery(RowMetaInterface parametersMeta, Object[] parameters) throws KettleDatabaseException {
		...
		if (parametersMeta.isEmpty()) {
			...
			data.rs = data.db.openQuery(sql, parametersMeta, parameters, ResultSet.FETCH_FORWARD,
					meta.isLazyConversionActive());
		}
		if (data.rs == null) {
	...
	public void stopRunning(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		meta = (TableInputVarcharMeta) smi;
		data = (TableInputVarcharData) sdi;
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			...
			if (Const.isEmpty(meta.getSQL())) {
				logError(BaseMessages.getString(PKG, "TableInputVarchar.Exception.SQLIsNeeded"));
				passed = false;
			}

			if (meta.getDatabaseMeta() == null) {
				logError(BaseMessages.getString(PKG, "TableInputVarchar.Exception.DatabaseConnectionsIsNeeded"));
				passed = false;
			}
			if (!passed) {
				return false;
			...
			if (meta.getDatabaseMeta() == null) {
				logError(BaseMessages.getString(PKG, "TableInputVarchar.Init.ConnectionMissing", getStepname()));
				return false;
			}
