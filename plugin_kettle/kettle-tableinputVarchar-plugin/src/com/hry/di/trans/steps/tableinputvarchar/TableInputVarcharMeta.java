...
package com.hry.di.trans.steps.tableinputvarchar;
...
@Step(id = "tableinputVarchar", image = "BC.png", i18nPackageName = "com.hry.di.trans.steps.tableinputvarchar", name = "TableInputVarcharDialog.TableInputVarchar", description = "TableInputVarCharDialog.Description", categoryDescription = "i18n:com.hry.di.trans.steps.tableinputvarchar:TableInputVarchar.Category.Haier")
public class TableInputVarcharMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = TableInputVarcharMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!
	...
	public Object clone() {
		TableInputVarcharMeta retval = (TableInputVarcharMeta) super.clone();
		return retval;
	}
		...
		if (add != null) {
			for (int i = 0; i < add.size(); i++) {
				...
				if (v.getType() == ValueMetaInterface.TYPE_STRING) {
					v.setLength(v.getLength() * 3);					
					// v.setOriginalPrecision(v.getLength() * 3);
				}
			}
			row.addRowMeta(add);
		} else {
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, "An error occurred: " + e.getMessage(),
						stepMeta);
				remarks.add(cr);
			} finally {
				db.disconnect();
		...
		if (!Const.isEmpty(infoStream.getStepname())) {
			...
			if (found) {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
						"Previous step to read info from [" + infoStream.getStepname() + "] is found.", stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						"Previous step to read info from [" + infoStream.getStepname() + "] is not found.", stepMeta);
				remarks.add(cr);
			}
			...
			if (info != null) {
				if (count == info.size()) {
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, "This step is expecting and receiving "
							+ info.size() + " fields of input from the previous step.", stepMeta);
					remarks.add(cr);
				} else {
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR, "This step is receiving " + info.size()
									+ " but not the expected " + count + " fields of input from the previous step.",
							stepMeta);
					remarks.add(cr);
				}
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, "Input step name is not recognized!",
						stepMeta);
				remarks.add(cr);
			}
		} else {
			if (input.length > 0) {
				...
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, "No input expected, no input provided.",
						stepMeta);
				remarks.add(cr);
			}
	...
	public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
			Trans trans) {
		return new TableInputVarchar(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData() {
		return new TableInputVarcharData();
	}
		...
		if (out != null) {
			for (int i = 0; i < out.size(); i++) {
				...
				DatabaseImpact ii = new DatabaseImpact(DatabaseImpact.TYPE_IMPACT_READ, transMeta.getName(),
						stepMeta.getName(), databaseMeta.getDatabaseName(), "", outvalue.getName(), outvalue.getName(),
						stepMeta.getName(), sql, "read from one or more database tables via SQL statement");
				impact.add(ii);
	...
	public StepIOMetaInterface getStepIOMeta() {
		if (ioMeta == null) {
			...
			StreamInterface stream = new Stream(StreamType.INFO, null,
					BaseMessages.getString(PKG, "TableInputVarcharMeta.InfoStream.Description"), StreamIcon.INFO, null);
	...
	@Override
	public TableInputVarcharMetaInjection getStepMetaInjectionInterface() {
		return new TableInputVarcharMetaInjection(this);
	}
