...
@Step(id = "insertupdatevar", image = "OBF.png", i18nPackageName = "com.hry.di.trans.steps.insertupdatevar", name = "InsertUpdateVarDialog.Shell.Title", description = "InsertUpdateVar.TransDescription", categoryDescription = "i18n:com.hry.di.trans.steps.insertupdatevar:InsertUpdateVar.Category.Haier")
public class InsertUpdateVarMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = InsertUpdateVarMeta.class; // for i18n //
																// purposes, //
																// needed by//
																// Translator2!!
	
	/** 数据库 what's the lookup schema? */
	private String schemaName;

	/** 表名 what's the lookup table? */
	private String tableName;

	/** 数据库元 database connection */
	private DatabaseMeta databaseMeta;

	/** 数据流key which field in input stream to compare with? */
	private String[] keyStream;

	/** 数据表key field in table */
	private String[] keyLookup;

	/** Comparator: =, <>, BETWEEN, ... */
	private String[] keyCondition;

	/** 数据流key2 Extra field for between... */
	private String[] keyStream2;

	/** 数据表更新字段 Field value to update after lookup */
	private String[] updateLookup;

	/** 数据流更新字段 Stream name to update value with */
	private String[] updateStream;

	/** 更新字段，是否更新 boolean indicating if field needs to be updated */
	private Boolean[] update;

	/** 提交数 Commit size for inserts/updates */
	private String commitSize;

	/** 是否执行更新操作 Bypass any updates */
	private boolean updateBypassed;

	public InsertUpdateVarMeta() {
	...
	public int getCommitSize(VariableSpace vs) {
		// this happens when the step is created via API and no setDefaults was
		// called
	...
	public Object clone() {
		InsertUpdateVarMeta retval = (InsertUpdateVarMeta) super.clone();
	...
	private void readData(Node stepnode, List<? extends SharedObjectInterface> databases) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(
					BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.UnableToReadStepInfoFromXML"), e);
		}
	}
	...
	public void setDefault() {
		...
		commitSize = "100";
		schemaName = "";
		tableName = BaseMessages.getString(PKG, "InsertUpdateVarMeta.DefaultTableName");
		...
		for (int i = 0; i < nrvalues; i++) {
			updateLookup[i] = BaseMessages.getString(PKG, "InsertUpdateVarMeta.ColumnName.ReturnField") + i;
			updateStream[i] = BaseMessages.getString(PKG, "InsertUpdateVarMeta.ColumnName.NewName") + i;
			update[i] = Boolean.TRUE;
		}
	}
	...
	public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases)
		...
		try {
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"InsertUpdateVarMeta.Exception.UnexpectedErrorReadingStepInfoFromRepository"), e);
		}
	}
	...
	public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step)
		...
		try {
			...
			throw new KettleException(
					BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.UnableToSaveStepInfoToRepository")
							+ id_step,
					e);
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
							BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.TableNameOK"), stepMeta);
					remarks.add(cr);
					...
					if (r != null) {
						cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
								BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.TableExists"), stepMeta);
						remarks.add(cr);
							...
							if (v == null) {
								if (first) {
									...
									error_message += BaseMessages.getString(PKG,
											"InsertUpdateVarMeta.CheckResult.MissingCompareFieldsInTargetTable")
											+ Const.CR;
								}
								error_found = true;
						...
						if (error_found) {
							...
							cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
									BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.AllLookupFieldsFound"),
									stepMeta);
						}
						remarks.add(cr);
						...
						for (int i = 0; i < updateLookup.length; i++) {
							...
							if (v == null) {
								if (first) {
									first = false;
									error_message += BaseMessages.getString(PKG,
											"InsertUpdateVarMeta.CheckResult.MissingFieldsToUpdateInTargetTable")
											+ Const.CR;
								}
								error_found = true;
								error_message += "\t\t" + lufield + Const.CR;
						...
						if (error_found) {
							...
							cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
									BaseMessages.getString(PKG,
											"InsertUpdateVarMeta.CheckResult.AllFieldsToUpdateFoundInTargetTable"),
									stepMeta);
						}
						remarks.add(cr);
					} else {
						error_message = BaseMessages.getString(PKG,
								"InsertUpdateVarMeta.CheckResult.CouldNotReadTableInfo");
						cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
						remarks.add(cr);
					}
				}
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
							"InsertUpdateVarMeta.CheckResult.StepReceivingDatas", prev.size() + ""), stepMeta);
					remarks.add(cr);
					...
					for (int i = 0; i < keyStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += BaseMessages.getString(PKG,
										"InsertUpdateVarMeta.CheckResult.MissingFieldsInInput") + Const.CR;
							}
							error_found = true;
					...
					if (error_found) {
						...
						cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
								BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.AllFieldsFoundInInput"),
								stepMeta);
					}
					remarks.add(cr);
					...
					for (int i = 0; i < updateStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += BaseMessages.getString(PKG,
										"InsertUpdateVarMeta.CheckResult.MissingInputStreamFields") + Const.CR;
							}
							error_found = true;
							error_message += "\t\t" + lufield + Const.CR;
						}
					...
					if (error_found) {
						...
						cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
								BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.AllFieldsFoundInInput2"),
								stepMeta);
					}
					remarks.add(cr);
				} else {
					error_message = BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.MissingFieldsInInput3")
							+ Const.CR;
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
					remarks.add(cr);
				}
			} catch (KettleException e) {
				error_message = BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.DatabaseErrorOccurred")
						+ e.getMessage();
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			} finally {
			...
			error_message = BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.InvalidConnection");
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.StepReceivingInfoFromOtherSteps"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.NoInputError"), stepMeta);
			remarks.add(cr);
		}
	}
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						if (keyLookup != null && keyLookup.length > 0) {
							...
							retval.setError(
									BaseMessages.getString(PKG, "InsertUpdateVarMeta.CheckResult.MissingKeyFields"));
						}
						...
						if (idx_fields != null && idx_fields.length > 0
							...
							cr_index = db.getCreateIndexStatement(schemaTable, indexname, idx_fields, false, false,
									false, true);
						}
						...
						retval.setError(BaseMessages.getString(PKG, "InsertUpdateVarMeta.ReturnValue.ErrorOccurred")
								+ e.getMessage());
					}
				} else {
					retval.setError(
							BaseMessages.getString(PKG, "InsertUpdateVarMeta.ReturnValue.NoTableDefinedOnConnection"));
				}
			} else {
				retval.setError(BaseMessages.getString(PKG, "InsertUpdateVarMeta.ReturnValue.NotReceivingAnyFields"));
			}
		} else {
			retval.setError(BaseMessages.getString(PKG, "InsertUpdateVarMeta.ReturnValue.NoConnectionDefined"));
		}
	...
	public void analyseImpact(List<DatabaseImpact> impact, TransMeta transMeta, StepMeta stepMeta,
		...
		if (prev != null) {
			...
			for (int i = 0; i < keyLookup.length; i++) {
				...
				DatabaseImpact ii = new DatabaseImpact(DatabaseImpact.TYPE_IMPACT_READ, transMeta.getName(),
						stepMeta.getName(), databaseMeta.getDatabaseName(), tableName, keyLookup[i], keyStream[i],
			...
			for (int i = 0; i < updateLookup.length; i++) {
				...
				DatabaseImpact ii = new DatabaseImpact(DatabaseImpact.TYPE_IMPACT_READ_WRITE, transMeta.getName(),
						stepMeta.getName(), databaseMeta.getDatabaseName(), tableName, updateLookup[i], updateStream[i],
						v != null ? v.getOrigin() : "?", "", "Type = " + v.toStringMeta());
				impact.add(ii);
			}
		}
	...
	public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
			Trans trans) {
		return new InsertUpdateVar(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData() {
		return new InsertUpdateVarData();
	}
			...
			try {
				...
				if (!Const.isEmpty(realTableName)) {
					...
					if (db.checkTableExists(schemaTable)) {
						...
						throw new KettleException(
								BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.TableNotFound"));
					}
				} else {
					throw new KettleException(
							BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.TableNotSpecified"));
				}
			} catch (Exception e) {
				throw new KettleException(
						BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.ErrorGettingFields"), e);
			} finally {
				db.disconnect();
			}
		} else {
			throw new KettleException(
					BaseMessages.getString(PKG, "InsertUpdateVarMeta.Exception.ConnectionNotDefined"));
		}
