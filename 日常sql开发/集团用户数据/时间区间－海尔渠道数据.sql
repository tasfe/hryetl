...
AND S0.RPTDT<20170501
GROUP BY S0.KJTID
),
TMP_USER_ACCOUNT_AMT AS (
select
*
from (
SELECT
...
,S0.TYPE_NAME
,S0.AFTER_AMT
FROM BIDATA.DW_USER_ACCT_BAL S0
where TO_DATE('20170430','yyyymmdd') < s0.end_date and TO_DATE('20170430','yyyymmdd')>s0.start_date
)
PIVOT (max(after_amt) for type_name in ('P2P' AS P2P_AMT,'ZH' AS ZH_AMT,'TTJ' AS TTJ_AMT))
)
...
LEFT JOIN TMP_USER_ACCOUNT_AMT S8
ON S1.HRYID=S8.HRYID
) T
