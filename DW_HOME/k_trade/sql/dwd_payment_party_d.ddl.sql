-- 无法呈现对象 PAYMENT 的 TABLE DDL。带有 DBMS_METADATA 的 TB_PARTY_PAYMENT 正在尝试内部生成器。

CREATE TABLE K_TRADE.DWD_PAYMENT_PARTY_D  
(
...
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.PAYMENT_PARTY_ID IS '会员支付信息ID，SEQ_MEMBER_PAYMENT_INFO';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.PAYMENT_SEQ_NO IS 'YYYYMMDD||CODE[2]||SEQ_PAYMENT_ORDER[8]';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.PARTY_ID_TYPE IS '01 现金账户/废除不用';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D
.PARTY_ID IS '会员标识，来源于帐户体系';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.ACCOUNT_NO IS '账号';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.FUND_PROP_TYPE IS '    DEBIT_FUND("DA", "借记资金"),
    CREDIT_FUND("CA", "贷记资金"),
    CHANNEL_FUND("PD", "依赖渠道资金属性");';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.ROLE_CODE IS '角色: PAYER,PAYEE,INST';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.AMOUNT IS '支付金额';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.CHARGE_FEE IS '收费金额';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.RETURN_FEE IS '退费金额';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.EXTENSION IS '扩展信息';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.GMT_CREATE IS '创建时间';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.GMT_MODIFIED IS '最后修改时间';
COMMENT ON TABLE K_TRADE.DWD_PAYMENT_PARTY_D IS '参与方支付信息';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.DW_CREATE_TIME IS 'DW 创建时间';
COMMENT ON COLUMN K_TRADE.DWD_PAYMENT_PARTY_D.DW_MODIFIED_TIME IS 'DW 修改时间';


CREATE INDEX K_TRADE.IDX_PAYMENT_PARTY_MODIFIED on K_TRADE.DWD_PAYMENT_PARTY_D(GMT_MODIFIED);
