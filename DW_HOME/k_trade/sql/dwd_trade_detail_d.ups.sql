--DELETE FROM "K_TRADE"."DWD_TRADE_DETAIL_D" WHERE TRADE_SOURCE = 'FOS'

--TSS
--ALTER TABLE "K_TRADE"."DWD_TRADE_DETAIL_D"  TRUNCATE PARTITION P201706 ;
...
FROM   
 (SELECT 
       ...
       WHERE 1=1
         AND to_char(d.GMT_SUBMIT,'YYYYMMDD') >= '20170601'   
      ...
      (
        SELECT 
               ...
               FROM K_TRADE.DWD_TSS_PAYMENT_ORDER_D d 
                WHERE d.GMT_CREATE >= to_date('20170601','YYYYMMDD'）  
          ...
          LEFT OUTER JOIN 
            (SELECT 
             ...
             FROM K_TRADE.DWD_TSS_PAY_METHOD_D 
              WHERE  GMT_CREATE >= to_date('20170601','YYYYMMDD')
             ) m
...
LEFT OUTER JOIN 
 (
  SELECT 
    ...
    , c.REMARK REAL_PAY_MODE_DESC
    ,d.GMT_CREATE CMF_GMT_CREATE
  FROM K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D d
  LEFT OUTER JOIN k_lkp.pay_mode c
    ON d.REAL_PAY_MODE = c.PAY_MODE
    WHERE  d.GMT_CREATE >= to_date('20170601','YYYYMMDD')
