CREATE TABLE K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D
(
	CMF_SEQ_NO VARCHAR2(32 BYTE) NOT NULL ENABLE
...
COMMENT ON TABLE K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D IS '支付渠道交易订单明细表';


COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.CMF_SEQ_NO IS '渠道流水号，YYYYMMDD+SEQ_CMF_ORDER';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.PAYMENT_SEQ_NO IS '支付流水号';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.ORDER_TYPE IS '订单类型：I（入款），B（退款），O（出款）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.PRODUCT_CODE IS '产品码';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.PAYMENT_CODE IS '支付编码';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.MEMBER_ID IS '会员ID,现阶段是PT帐号';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.AMOUNT IS '金额';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.CURRENCY IS '币种';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.TARGET_INST_CODE IS '目标机构编码';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.PAYMENT_NOTIFY_STATUS IS '支付结果通知状态：S（通知成功），F（通知失败），N（不通知）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.CMF_ORDER_STATUS IS 'CMF 订单状态：A（待处理），C（已撤销），I（处理中），S（成功），F（失败）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.INST_ORDER_ID IS '机构订单ID';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.BIZ_DATE IS '业务发生时间';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.GMT_CREATE IS 'CMF 订单创建时间';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.GMT_MODIFIED IS 'CMF 订单最后修改时间';
--COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.CONFIRM_STATUS IS '';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.DECLARE_PAY_MODE IS '对外声明支付模式';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.SETTLEMENT_ID IS '结算ID';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.INST_ORDER_NO IS '机构订单号';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.INST_ORDER_STATUS IS '状态：I（处理中），S（成功），F（失败）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.COMMUNICATE_TYPE IS '通讯类型，S（单笔通信），B（批量通信）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.COMMUNICATE_STATUS IS '通讯类型，类型为单笔时：A（等待指令发送），S（指令已经发送），R（数据已经返回），F（指令发送失败）。批量时：A（等待数据归档），G（归档已经生成），S（归档数据已经提交），R（归档数据已经返回），F（归档数据提交失败）';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.INST_ORDER_MODIFIED_TIME IS '机构订单 最后修改时间';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.MEMO IS '机构订单 备注';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FUND_CHANNEL_CODE IS '资金渠道编码';
--COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FUND_CHANNEL_API IS '';
--COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FLAG IS '';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FUND_CHANNEL_NAME IS '支付渠道名称';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FUND_CHANNEL_DES IS '支付渠道描述';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.FUND_CHANNEL_INST_CODE IS '支付渠道归属机构编码';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.REAL_PAY_MODE IS '实际支付模式';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.IS_BFJ IS '是否为备付金调拨';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.CONNECTTYPE IS '资金渠道连接类型';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.DW_CREATE_TIME IS 'DW 创建时间';
COMMENT ON COLUMN K_CHANNEL.DWD_FUND_CHANNEL_ORDER_D.DW_MODIFIED_TIME IS 'DW 更新时间';
;
